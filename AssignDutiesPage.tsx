...
// ===== Section Definitions =====
const SECTIONS = [
  { id: 'A', nameKey: 'assignDuties.sections.sectionA', descKey: 'assignDuties.sections.sectionADesc', color: '#312e81', bgColor: '#eef2ff' },
  { id: 'B', nameKey: 'assignDuties.sections.sectionB', descKey: 'assignDuties.sections.sectionBDesc', color: '#0d9488', bgColor: '#f0fdfa' },
  { id: 'C', nameKey: 'assignDuties.sections.sectionC', descKey: 'assignDuties.sections.sectionCDesc', color: '#7c3aed', bgColor: '#f5f3ff' },
  { id: 'D', nameKey: 'assignDuties.sections.sectionD', descKey: 'assignDuties.sections.sectionDDesc', color: '#dc2626', bgColor: '#fef2f2' },
  { id: 'E', nameKey: 'assignDuties.sections.sectionE', descKey: 'assignDuties.sections.sectionEDesc', color: '#d97706', bgColor: '#fffbeb' },
  { id: 'F', nameKey: 'assignDuties.sections.sectionF', descKey: 'assignDuties.sections.sectionFDesc', color: '#2563eb', bgColor: '#eff6ff' },
  { id: 'G', nameKey: 'assignDuties.sections.sectionG', descKey: 'assignDuties.sections.sectionGDesc', color: '#059669', bgColor: '#ecfdf5' },
  { id: 'O', nameKey: 'assignDuties.sections.sectionO', descKey: 'assignDuties.sections.sectionODesc', color: '#be185d', bgColor: '#fdf2f8' },
  { id: 'MT', nameKey: 'assignDuties.sections.sectionMT', descKey: 'assignDuties.sections.sectionMTDesc', color: '#ea580c', bgColor: '#fff7ed' },
  { id: 'TRAINING', nameKey: 'assignDuties.sections.sectionTraining', descKey: 'assignDuties.sections.sectionTrainingDesc', color: '#4338ca', bgColor: '#eef2ff' },
];

// ===== Leave Categories (from Form 168) =====
const LEAVE_CATEGORIES = [
  { name: 'CL (CASUAL LEAVE)', nameKey: 'assignDuties.leaveCategories.casualLeave', descKey: 'assignDuties.leaveCategories.casualLeaveDesc' },
  { name: 'CML (COMMUTED LEAVE)', nameKey: 'assignDuties.leaveCategories.compensatoryLeave', descKey: 'assignDuties.leaveCategories.compensatoryLeaveDesc' },
  { name: 'PL (PATERNITY LEAVE)', nameKey: 'assignDuties.leaveCategories.privilegeLeave', descKey: 'assignDuties.leaveCategories.privilegeLeaveDesc' },
  { name: 'EL (EARNED LEAVE)', nameKey: 'assignDuties.leaveCategories.earnedLeave', descKey: 'assignDuties.leaveCategories.earnedLeaveDesc' },
  { name: 'ABSENT', nameKey: 'assignDuties.leaveCategories.absent', descKey: 'assignDuties.leaveCategories.absentDesc' },
  { name: 'SUSPENSION', nameKey: 'assignDuties.leaveCategories.suspension', descKey: 'assignDuties.leaveCategories.suspensionDesc' },
  { name: 'PERMISSION / RH TO RPI, RSI AND', nameKey: 'assignDuties.leaveCategories.permission', descKey: 'assignDuties.leaveCategories.permissionDesc' },
  { name: 'SICK', nameKey: 'assignDuties.leaveCategories.sick', descKey: 'assignDuties.leaveCategories.sickDesc' },
  { name: 'WEEKLY OFF', nameKey: 'assignDuties.leaveCategories.weeklyOff', descKey: 'assignDuties.leaveCategories.weeklyOffDesc' },
];

// Global CSS keyframes for highlighting animation
const globalStyles = `
  @keyframes pulse-highlight-global {
    0%, 100% { box-shadow: 0 0 12px rgba(234, 179, 8, 0.4); }
    50% { box-shadow: 0 0 20px rgba(234, 179, 8, 0.6); }
  }
  .highlight-pulse {
    animation: pulse-highlight-global 1s ease-in-out infinite;
  }
`;
...
const AssignDutiesPage: React.FC = () => {
  ...
  const [globalSearchResults, setGlobalSearchResults] = useState<PersonnelSearchResult[]>([]);
  ...
  const handleGlobalSearch = useCallback(async (query: string) => {
    ...
    try {
      const results = await sectionChartService.searchPersonnel(query, 15);
  ...
  const handleGlobalSearchSelect = useCallback(async (person: PersonnelSearchResult | null) => {
    if (!person) return;
    setGlobalSearchQuery('');
    setGlobalSearchResults([]);
    ...
    try {
      const leaveResult = await leaveService.list(undefined, 0, 100);
      const activeLeave = leaveResult.content.find(
        (lr) => lr.personnelId === person.id && 
                lr.status === 'APPROVED' && 
                lr.startDate <= today && 
                (!lr.endDate || lr.endDate >= today)
      );
      
      if (activeLeave) {
        // Person is on leave - navigate to Leave Management tab
        const sectionData = SECTIONS.find(s => s.id === person.currentSection);
        if (sectionData) {
          setSelectedSection(person.currentSection);
          setSearchParams({ section: person.currentSection, tab: '1' }, { replace: true });
        }
        setActiveTab(1);
        
        // Find the leave category to select
        const displayName = BACKEND_TO_DISPLAY[activeLeave.leaveType] || activeLeave.leaveType;
        if (LEAVE_CATEGORIES.find(c => c.name === displayName)) {
          setSelectedLeaveCategory(displayName);
        }
        
        // Highlight the person and scroll into view
        setHighlightedPersonId(person.id);
        highlightTimeoutRef.current = setTimeout(() => setHighlightedPersonId(null), 5000);
        
        // Scroll to highlighted element after a short delay for React to render
        setTimeout(() => {
          const highlightedRow = document.querySelector('.highlight-pulse');
          if (highlightedRow) {
            highlightedRow.scrollIntoView({ behavior: 'smooth', block: 'center' });
          }
        }, 300);
        
        setSnackbar({ open: true, message: `Found ${person.personName} on ${activeLeave.leaveType.replace(/_/g, ' ')}`, severity: 'success' });
        return;
      }
    } catch {
      // Continue with duty management navigation
    }
    
    // Person is not on leave - navigate to their section and duty
    if (person.currentSection) {
      setSelectedSection(person.currentSection);
      setSearchParams({ section: person.currentSection, tab: '0' }, { replace: true });
      setActiveTab(0);
      
      // Load chart data and find the duty type
      try {
        const data = await sectionChartService.getChart(person.currentSection);
        setChartData(data);
        
        // Find the duty type that contains this person
        for (const card of data.dutyCards) {
          // Check main personnel
          const foundInMain = card.personnel.find(p => p.id === person.id);
          if (foundInMain) {
            setSelectedDutyTypeId(card.dutyTypeId);
            break;
          }
          // Check sub-duties
          for (const sub of card.subDuties) {
            const foundInSub = sub.personnel.find(p => p.id === person.id);
            if (foundInSub) {
              setSelectedDutyTypeId(card.dutyTypeId);
              break;
            }
        ...
        setSnackbar({ open: true, message: `Found ${person.personName} in Section ${person.currentSection}`, severity: 'success' });
      } catch {
        setSnackbar({ open: true, message: 'Failed to load section data', severity: 'error' });
      }
    } else {
      setSnackbar({ open: true, message: `${person.personName} is not assigned to any section`, severity: 'info' });
    }
  }, [setSearchParams]);
  ...
  return (
    <>
      {/* Global CSS for highlighting animation */}
      ...
      <Box sx={{ bgcolor: 'background.paper', borderBottom: '1px solid', borderColor: 'divider' }}>
        <Box sx={{ px: { xs: 2, md: 3 }, pt: 2, pb: 1.5 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={2}>
            ...
            <Box>
              <Stack direction="row" spacing={0.5} alignItems="center" sx={{ mb: 0.5 }}>
                ...
                <Typography sx={{ fontSize: '0.75rem', color: 'text.secondary' }}>
                  {t('nav.groups.operations', 'Operations')}
                </Typography>
                <Typography sx={{ fontSize: '0.75rem', color: 'text.disabled' }}>{'>'}</Typography>
                <Typography sx={{ fontSize: '0.75rem', color: 'text.primary', fontWeight: 500 }}>
                  {t('nav.assignDuties', 'Assign Duties')}
                </Typography>
              </Stack>
            ...
            {/* Right: Global Personnel Search - Google-style unified box */}
            <Box sx={{ flexShrink: 0, position: 'relative', width: 380 }}>
              <Paper
                elevation={globalSearchQuery.length >= 2 || globalSearchLoading ? 3 : 0}
                sx={{
                  borderRadius: (globalSearchQuery.length >= 2 || globalSearchLoading) ? '20px 20px 0 0' : '20px',
                  border: (globalSearchQuery.length >= 2 || globalSearchLoading) ? 'none' : '1px solid #dfe1e5',
                  overflow: 'hidden',
                  transition: 'box-shadow 0.2s, border-radius 0.2s',
                  '&:hover': {
                    boxShadow: (globalSearchQuery.length >= 2 || globalSearchLoading) ? undefined : '0 1px 6px rgba(32,33,36,0.28)',
                    borderColor: (globalSearchQuery.length >= 2 || globalSearchLoading) ? undefined : 'transparent',
                  },
                }}
              >
                {/* Search Input */}
                ...
                >
                  ...
                  <input
                    ...
                    placeholder={t('assignDuties.searchPlaceholder', 'Search personnel by name or badge ID...')}
                    style={{
                      flex: 1,
                      border: 'none',
                      outline: 'none',
                  ...
                  />
                  {globalSearchLoading && <CircularProgress size={18} sx={{ color: '#9aa0a6', ml: 1 }} />}
                  ...
                  {globalSearchQuery.length < 2 ? (
                    <Box sx={{ py: 2, px: 2.5 }}>
                      <Typography sx={{ fontSize: '0.85rem', color: '#70757a' }}>
                        {t('assignDuties.personnel.typeAtLeast', 'Type at least 2 characters...')}
                      </Typography>
                    </Box>
                  ) : globalSearchLoading ? (
                    ...
                    <Box sx={{ py: 0.5 }}>
                      {globalSearchResults.map((person) => (
                        <Box
                          key={person.id}
                          onClick={() => handleGlobalSearchSelect(person)}
                          sx={{
                            display: 'flex',
                            alignItems: 'center',
                            px: 2,
                        ...
                        >
                          <Box
                            sx={{
                              ...
                              height: 32,
                              borderRadius: '8px',
                              bgcolor: person.currentSection ? (SECTIONS.find(s => s.id === person.currentSection)?.bgColor || '#f1f5f9') : '#f1f5f9',
                              display: 'flex',
                              alignItems: 'center',
                              justifyContent: 'center',
                              mr: 1.5,
                          ...
                          >
                            <Typography sx={{ fontSize: '0.65rem', fontWeight: 700, color: person.currentSection ? (SECTIONS.find(s => s.id === person.currentSection)?.color || '#64748b') : '#64748b' }}>
                              {person.currentSection || '—'}
                            </Typography>
                          </Box>
                          <Box sx={{ flex: 1, minWidth: 0 }}>
                            ...
                            >
                              {person.badgeId} • {person.designation || 'No designation'} • {person.currentDutyType || 'Unassigned'}
                            </Typography>
                          </Box>
                          {person.designation && (
                            <Chip
                              ...
                              sx={{
                                ...
                                fontSize: '0.65rem',
                                fontWeight: 600,
                                bgcolor: person.currentSection ? `${SECTIONS.find(s => s.id === person.currentSection)?.color}15` : '#f1f5f9',
                                color: person.currentSection ? (SECTIONS.find(s => s.id === person.currentSection)?.color || '#64748b') : '#64748b',
                                '& .MuiChip-label': { px: 1 },
                              }}
                            />
                          )}
                    ...
                    <Box sx={{ py: 2.5, px: 2.5 }}>
                      <Typography sx={{ fontSize: '0.85rem', color: '#70757a', textAlign: 'center' }}>
                        {t('assignDuties.personnel.noPersonnelFound', 'No personnel found')} "{globalSearchQuery}"
                      </Typography>
                    </Box>
                  )}
                </Paper>
      ...
      <Box sx={{ px: { xs: 2, md: 3 }, pt: 3, pb: 1 }}>
        <Typography variant="subtitle1" sx={{ mb: 1.5, color: 'text.secondary', fontWeight: 600 }}>
          {t('assignDuties.selectSection', 'Select Section')}
        </Typography>
      ...
      <Box sx={{ flex: 1, px: { xs: 1, md: 3 }, py: 2 }}>
        ...
        {selectedData && chartData && (
          <Box sx={{ mb: 2, display: 'inline-flex', bgcolor: '#f1f5f9', borderRadius: '10px', p: '4px', gap: '4px' }}>
            {[
              { label: t('assignDuties.tabs.dutyManagement', 'Duty Management'), idx: 0 },
              { label: t('assignDuties.tabs.leaveManagement', 'Leave Management'), idx: 1 },
              { label: t('assignDuties.tabs.form168View', 'Form 168 View'), idx: 2 }
            ].map(({ label, idx }) => (
              <Box
                key={label}
                onClick={() => { setActiveTab(idx); setGlobalSearchQuery(''); setGlobalSearchResults([]); setSearchParams({ section: selectedSection || 'A', tab: String(idx) }, { replace: true }); }}
                sx={{
                  px: 2.5,
                  py: 1,
                  borderRadius: '8px',
        ...
        {loading ? (
            ...
            >
              ...
              <Box sx={{ px: 2, py: 1.5, bgcolor: selectedData.bgColor, borderBottom: '1px solid', borderColor: 'divider' }}>
                <Typography variant="subtitle2" sx={{ fontWeight: 700, color: selectedData.color }}>
                  {t(selectedData.nameKey)} {t('assignDuties.sidebar.duties')}
                </Typography>
              ...
              <Box sx={{ px: 1.5, py: 1 }}>
                <TextField
                  size="small"
                  fullWidth
                  placeholder={t('assignDuties.sidebar.filterPlaceholder')}
                  value={sidebarFilter}
                    ...
                    <ListItemText
                      primary={
                        <Stack direction="row" spacing={0.5} alignItems="center">
                          ...
                          >
                            {t('assignDuties.sidebar.reassignedDuties', 'Reassigned Duties')}
                          </Typography>
                          <Chip
                ...
                {showAddDutyType ? (
                  <Stack spacing={1}>
                    <TextField
                      size="small"
                      fullWidth
                      placeholder={t('assignDuties.sidebar.parentDutyTypeName')}
                    ...
                    <TextField
                      ...
                      fullWidth
                      type="number"
                      label={t('assignDuties.sidebar.sequenceNo')}
                      ...
                      helperText={t('assignDuties.sidebar.range', { max: (chartData?.dutyCards.length ?? 0) + 1 })}
                      InputProps={{ sx: { fontSize: '0.8rem' } }}
                    />
                  ...
                  >
                    {t('assignDuties.sidebar.addDutyType', 'Add Duty Type')}
                  </Button>
                )}
              </Box>
            </Paper>
                  ...
                  <Box sx={{ px: 3, py: 2, bgcolor: '#eff6ff', borderBottom: '1px solid', borderColor: 'divider' }}>
                    <Stack direction="row" spacing={1.5} alignItems="center">
                      <Typography variant="h6" sx={{ fontWeight: 700, color: '#1d4ed8', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                        {t('assignDuties.sidebar.reassignedDuties')}
                      </Typography>
                      <Chip label={`${activeReassignments.length} ${t('app.active').toLowerCase()}`} size="small" sx={{ fontWeight: 600, bgcolor: '#dbeafe', color: '#1d4ed8' }} />
                    </Stack>
                    <Typography variant="caption" color="text.secondary">{t('assignDuties.reassignment.personnelCovering')}</Typography>
                  </Box>
                  <Box sx={{ flex: 1, overflowY: 'auto', p: 3 }}>
                    {activeReassignments.length === 0 ? (
                      <Typography variant="body2" color="text.secondary">{t('assignDuties.reassignment.noActiveReassignments')}</Typography>
                    ) : (
                      <Stack spacing={1}>
                          ...
                          <Box key={r.id} sx={{ p: 1.5, borderRadius: '8px', border: '1px solid #dbeafe', bgcolor: '#f8faff' }}>
                            <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                              <Box>
                                ...
                                <Typography variant="caption" color="text.secondary">
                                  {t('assignDuties.reassignment.coveringFor', { badge: r.leavePersonBadgeId, name: r.leavePersonName })}
                                </Typography>
                              ...
                              <Stack direction="row" spacing={0.5} alignItems="center">
                                <Tooltip title={t('assignDuties.reassignment.editReassignment')}>
                                  <IconButton
                                    size="small"
                                ...
                                <Tooltip title={t('assignDuties.reassignment.cancelReassignment')}>
                                  <IconButton
                                    size="small"
                                    onClick={async () => {
                                      try {
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', opacity: 0.5 }}>
                  <Stack alignItems="center" spacing={1}>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {t('assignDuties.dutyPanel.selectDutyType')}
                    </Typography>
                  </Stack>
                </Box>
              )}
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', py: 8, gap: 2 }}>
            ...
            <Typography variant="h6" color="text.secondary" fontWeight={600}>
              {t('assignDuties.noSectionSelected.title')}
            </Typography>
            <Typography variant="body2" color="text.disabled">
              {t('assignDuties.noSectionSelected.subtitle')}
            </Typography>
          </Box>
        )}
      </Box>
      ...
      <Dialog open={deleteDialog.open} onClose={() => setDeleteDialog((d) => ({ ...d, open: false }))}>
        <DialogTitle>{t('assignDuties.confirmDelete.title')}</DialogTitle>
        <DialogContent>
          <DialogContentText>
            {deleteDialog.type === 'dutyType' 
              ? t('assignDuties.confirmDelete.dutyTypeMessage', { name: deleteDialog.name })
              : t('assignDuties.confirmDelete.subDutyMessage', { name: deleteDialog.name })}
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDeleteDialog((d) => ({ ...d, open: false }))}>{t('app.cancel')}</Button>
          <Button onClick={confirmDelete} color="error" variant="contained">{t('app.delete')}</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle sx={{ fontWeight: 700 }}>{reassignDialog.editMode ? t('assignDuties.reassignment.editTitle') : t('assignDuties.reassignment.title')}</DialogTitle>
        <DialogContent>
          {reassignDialog.person && (
            <Stack spacing={2.5} sx={{ mt: 1 }}>
              {/* Leave Person Info */}
              <Box sx={{ p: 1.5, borderRadius: '8px', bgcolor: '#fee2e2', border: '1px solid #fca5a5' }}>
                <Typography variant="caption" color="text.secondary" fontWeight={600}>{t('assignDuties.reassignment.personOnLeave')}</Typography>
                <Typography variant="body2" fontWeight={700}>
                  {reassignDialog.person.badgeId} — {reassignDialog.person.personName}
                </Typography>
                <Typography variant="caption" color="text.secondary">
                  {t('assignDuties.reassignment.leaveInfo', { start: reassignDialog.person.leaveStartDate, end: reassignDialog.person.leaveEndDate, days: reassignDialog.person.leaveDays })}
                </Typography>
              </Box>
              ...
              <Box>
                <Typography variant="caption" fontWeight={600} sx={{ mb: 0.5, display: 'block' }}>
                  {reassignDialog.editMode ? t('assignDuties.reassignment.changeReplacement') : t('assignDuties.reassignment.selectReplacement')}
                </Typography>
                {reassignDialog.editMode && (
                  <Box sx={{ p: 1, mb: 1, borderRadius: '6px', bgcolor: '#dbeafe', border: '1px solid #93c5fd' }}>
                    <Typography variant="caption" color="text.secondary">{t('assignDuties.reassignment.currentReplacement')}</Typography>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      {t('assignDuties.reassignment.searchToChange')}
                    </Typography>
                  </Box>
                )}
              ...
              <Stack direction="row" spacing={2}>
                <TextField
                  label={t('assignDuties.reassignment.startDate')}
                  type="date"
                  size="small"
                  fullWidth
                  ...
                  helperText={t('assignDuties.reassignment.cannotExceedLeave')}
                />
                <TextField
                  label={t('assignDuties.reassignment.endDate')}
                  type="date"
                  size="small"
                  fullWidth
              ...
              {reassignForm.startDate && reassignForm.endDate && reassignDialog.person.leaveEndDate && (
                ...
                <Alert severity="error" sx={{ fontSize: '0.75rem' }}>
                  {t('assignDuties.reassignment.datesValidation', { start: reassignDialog.person.leaveStartDate, end: reassignDialog.person.leaveEndDate })}
                </Alert>
              )}
            </Stack>
          )}
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setReassignDialog({ open: false, person: null })}>{t('app.cancel')}</Button>
          <Button
            variant="contained"
          ...
          >
            {reassignSaving ? t('app.saving') : reassignDialog.editMode ? t('app.update') : t('assignDuties.reassignment.reassign')}
          </Button>
        </DialogActions>
      </Dialog>
...
const DutyDetailPanel: React.FC<DutyDetailPanelProps> = ({
  card, sectionCode, sectionColor, sectionBgColor, onAssign, onUnassign, onAddSubDuty, onDeleteDutyType, onDeleteSubDuty, onRenameDutyType, onRenameSubDuty, onReassign, reassignments, highlightedPersonId,
}) => {
  const { t } = useTranslation();
          ...
          <Stack direction="row" spacing={1.5} alignItems="center">
            {editingParentName ? (
              <Stack direction="row" spacing={0.5} alignItems="center">
                ...
                <Button size="small" variant="contained" onClick={handleRenameParent} sx={{ fontSize: '0.7rem', minWidth: 'auto' }}>{t('app.save')}</Button>
                <Button size="small" onClick={() => setEditingParentName(false)} sx={{ fontSize: '0.7rem', minWidth: 'auto' }}>{t('app.cancel')}</Button>
              </Stack>
            ) : (
            ...
            <Chip
              label={`${card.personnelCount} ${t('assignDuties.dutyPanel.personnel')}`}
              size="small"
              sx={{ fontWeight: 600, bgcolor: `${sectionColor}15`, color: sectionColor }}
            />
          </Stack>
          <Stack direction="row" spacing={0.5}>
            <Tooltip title={t('assignDuties.dutyPanel.editDutyTypeName')}>
            ...
            <Tooltip title={t('assignDuties.dutyPanel.deleteDutyType')}>
          ...
          <Box sx={{ mt: card.subDuties.length > 0 ? 3 : 0 }}>
            {card.subDuties.length > 0 && (
              <Typography variant="caption" sx={{ fontWeight: 600, color: 'text.secondary', mb: 1, display: 'block', textTransform: 'uppercase' }}>
                {t('assignDuties.dutyPanel.directAssignment')}
              </Typography>
            )}
            <PersonnelList
        ...
        {card.subDuties.length === 0 && card.personnel.length === 0 && (
          <Box sx={{ py: 2 }}>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              {t('assignDuties.dutyPanel.noPersonnelAssigned')}
            </Typography>
            <PersonnelList
              personnel={[]}
              dutyTypeId={card.dutyTypeId}
          ...
          {showAddSubDuty ? (
            <Stack spacing={1}>
              <Stack direction="row" spacing={1} alignItems="center">
                <TextField
                  size="small"
                  placeholder={t('assignDuties.dutyPanel.subDutyName')}
                ...
                <TextField
                  size="small"
                  placeholder={t('assignDuties.dutyPanel.locationOptional')}
            ...
            <Button
              ...
              sx={{ textTransform: 'none' }}
            >
              {t('assignDuties.dutyPanel.addSubDuty')}
            </Button>
          )}
        </Box>
      </Box>
...
const SubDutySection: React.FC<SubDutySectionProps> = ({ subDuty, dutyTypeId, sectionColor, onAssign, onUnassign, onDeleteSubDuty, onRenameSubDuty, onReassign, reassignments, highlightedPersonId }) => {
  const { t } = useTranslation();
      ...
      <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
        {editing ? (
          <Stack direction="row" spacing={0.5} alignItems="center" sx={{ flex: 1 }}>
            ...
            <Button size="small" variant="contained" onClick={handleRename} sx={{ fontSize: '0.65rem', minWidth: 'auto' }}>{t('app.save')}</Button>
            <Button size="small" onClick={() => setEditing(false)} sx={{ fontSize: '0.65rem', minWidth: 'auto' }}>{t('app.cancel')}</Button>
          </Stack>
        ) : (
...
const PersonnelList: React.FC<PersonnelListProps> = ({ personnel, dutyTypeId, subDuty, onAssign, onUnassign, onReassign, reassignments, highlightedPersonId }) => {
  const { t } = useTranslation();
  ...
  return (
    <Box>
      ...
      {showSearch ? (
        ...
        >
          {t('assignDuties.personnel.addPerson', 'Add Person')}
        </Button>
      )}
    </Box>
  );
...
interface PersonnelRowProps {
  ...
  isHighlighted?: boolean; // for global search highlighting
}

const PersonnelRow: React.FC<PersonnelRowProps> = ({ person, onRemove, onReassign, reassignedTo, isHighlighted }) => {
  const { t } = useTranslation();
  return (
    <Box
      ...
      sx={{
        ...
        py: 0.6,
        borderRadius: '8px',
        bgcolor: isHighlighted ? '#fef9c3' : person.onLeave ? '#fff5f5' : 'grey.50',
        border: isHighlighted ? '2px solid #eab308' : '1px solid',
        borderColor: isHighlighted ? '#eab308' : person.onLeave ? '#fca5a5' : 'grey.200',
        transition: 'all 0.15s',
        animation: isHighlighted ? 'pulse-highlight 1s ease-in-out infinite' : 'none',
        boxShadow: isHighlighted ? '0 0 12px rgba(234, 179, 8, 0.4)' : 'none',
        '@keyframes pulse-highlight': {
          '0%, 100%': { boxShadow: '0 0 12px rgba(234, 179, 8, 0.4)' },
          '50%': { boxShadow: '0 0 20px rgba(234, 179, 8, 0.6)' },
        },
        '&:hover': { bgcolor: isHighlighted ? '#fef08a' : person.onLeave ? '#fee2e2' : 'grey.100', borderColor: isHighlighted ? '#ca8a04' : person.onLeave ? '#f87171' : 'grey.300', '& .remove-btn': { opacity: 1 } },
      }}
    >
      <Stack direction="row" spacing={1} alignItems="center" sx={{ minWidth: 0, flex: 1 }}>
        ...
        {person.onLeave && (
          <Chip
            label={`${t('assignDuties.personnel.onLeave')} · ${person.leaveType?.replace(/_/g, ' ')} (${person.leaveDays}d)`}
            size="small"
        ...
        {person.onLeave && reassignedTo && (
          <Chip
            label={t('assignDuties.personnel.reassignedTo', { badge: reassignedTo })}
            size="small"
        ...
        {(person as any).onAdhocDuty && (
          <Chip
            label={t('assignDuties.personnel.adhocDuty', { name: (person as any).adhocDutyName, start: (person as any).adhocDutyStartTime, end: (person as any).adhocDutyEndTime })}
            size="small"
      ...
      <Stack direction="row" spacing={0.5} alignItems="center">
        {person.onLeave && onReassign && !reassignedTo && (
          ...
          >
            {t('assignDuties.reassignment.reassign')}
          </Button>
        )}
        {person.onLeave && onReassign && reassignedTo && (
          <Button
        ...
        <Tooltip title={t('assignDuties.personnel.removeFromDuty')}>
          <IconButton
            size="small"
...
interface PersonnelAutocompleteProps {
  ...
  onSelectWithDetails?: (personnelId: number, personName: string, badgeId: string) => void;
  onCancel: () => void;
}

const PersonnelAutocomplete: React.FC<PersonnelAutocompleteProps> = ({ onSelect, onSelectWithDetails, onCancel }) => {
  const { t } = useTranslation();
  ...
  const handleSelect = (person: PersonnelSearchResult) => {
    ...
    if (onSelectWithDetails) {
      onSelectWithDetails(person.id, person.personName, person.badgeId);
    }
    setInputValue('');
    setOptions([]);
    setIsOpen(false);
  ...
  return (
    <Box sx={{ position: 'relative', width: '100%' }}>
      <Paper
        elevation={showDropdown && (loading || hasResults || inputValue.length >= 2) ? 2 : 0}
        sx={{
          borderRadius: showDropdown && (loading || hasResults || inputValue.length >= 2) ? '16px 16px 0 0' : '16px',
      ...
      >
        ...
        >
          ...
          <input
            ...
            placeholder={t('assignDuties.personnel.searchPlaceholder')}
            style={{
              flex: 1,
              border: 'none',
              outline: 'none',
          ...
          />
          {loading && <CircularProgress size={16} sx={{ color: '#9ca3af', mr: 0.5 }} />}
          <Box
            onClick={onCancel}
            sx={{
              cursor: 'pointer',
      ...
      {/* Results Dropdown - Absolutely positioned to overlay content */}
      {showDropdown && (inputValue.length >= 2 || loading) && (
        <Paper
          elevation={4}
          sx={{
            position: 'absolute',
          ...
          {inputValue.length < 2 ? (
            <Box sx={{ py: 1.5, px: 2 }}>
              <Typography sx={{ fontSize: '0.8rem', color: '#9ca3af' }}>
                {t('assignDuties.personnel.typeAtLeast')}
              </Typography>
            </Box>
          ) : loading ? (
              ...
              {options.map((person, index) => (
                <Box
                  ...
                  sx={{
                    ...
                    py: 1,
                    cursor: 'pointer',
                    bgcolor: focusedIndex === index ? '#f1f5f9' : 'transparent',
                ...
                >
                  ...
                  >
                    <BadgeIcon sx={{ fontSize: 14, color: '#7c3aed' }} />
                  </Box>
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    <Typography sx={{ fontSize: '0.82rem', fontWeight: 500, color: '#1e293b' }}>
                      {person.personName}
                    </Typography>
            ...
            <Box sx={{ py: 1.5, px: 2 }}>
              <Typography sx={{ fontSize: '0.8rem', color: '#9ca3af', textAlign: 'center' }}>
                {t('assignDuties.personnel.noPersonnelFound')}
              </Typography>
            </Box>
          )}
        </Paper>
...
const LEAVE_TYPE_MAP: Record<string, string> = {
  ...
  'CML (COMMUTED LEAVE)': 'COMMUTED_LEAVE',
  'PL (PATERNITY LEAVE)': 'PATERNITY_LEAVE',
...
const BACKEND_TO_DISPLAY: Record<string, string> = {
  ...
  'COMMUTED_LEAVE': 'CML (COMMUTED LEAVE)',
...
const LeaveManagementPanel: React.FC<LeaveManagementPanelProps> = ({
  ...
  selectedCategory, onCategoryChange, chartData, highlightedPersonId,
}) => {
  const { t } = useTranslation();
  ...
  const [formData, setFormData] = useState<{
    ...
    endDate: string;
    reason: string;
  }>({ personnelId: null, personnelName: '', startDate: '', endDate: '', reason: '' });

  // Close Leave state
  const [closeLeaveRecord, setCloseLeaveRecord] = useState<LeaveRequestDto | null>(null);
  ...
  const fetchLeaveRecords = useCallback(async () => {
    ...
    try {
      ...
      // Filter for APPROVED leaves where today is between startDate and endDate (or endDate is null/undefined = ongoing)
      const activeLeaves = result.content.filter((lr) => {
        const isApproved = lr.status === 'APPROVED';
        const startOk = lr.startDate <= today;
        // Handle both null and undefined for open-ended leaves
        const endOk = !lr.endDate || lr.endDate >= today;
        return isApproved && startOk && endOk;
      });
      // Filter by section personnel IDs (if available)
  ...
  const handleAddLeaveType = async () => {
    ...
    try {
      ...
      if (!LEAVE_CATEGORIES.find(c => c.name === upperName)) {
        // For dynamically added categories, use the name as display (no translation key)
        LEAVE_CATEGORIES.push({ name: upperName, nameKey: '', descKey: '' });
      }
    } catch {
  ...
  const handleSubmitLeave = async () => {
    // Allow all leave types to be submitted without end date (open-ended)
    if (!formData.personnelId || !formData.startDate) return;
    setSaving(true);
    try {
      ...
      setFormData({ personnelId: null, personnelName: '', startDate: '', endDate: '', reason: '' });
      fetchLeaveRecords();
    } catch {
  ...
  // Handle closing an open-ended leave
  const handleCloseLeaveConfirm = async () => {
    if (!closeLeaveRecord || !closeLeaveEndDate) return;
    setSaving(true);
    try {
      await leaveService.close(closeLeaveRecord.id, closeLeaveEndDate, closeLeaveComments);
      setCloseLeaveRecord(null);
    ...
    <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
      ...
      >
        ...
        <Box sx={{ px: 2, py: 1.5, bgcolor: sectionBgColor, borderBottom: '1px solid', borderColor: 'divider' }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 700, color: sectionColor }}>
            {t('assignDuties.leavePanel.title')}
          </Typography>
          <Typography variant="caption" color="text.secondary">
            {LEAVE_CATEGORIES.length} {t('assignDuties.leavePanel.types')} • {totalLeavePersonnel} {t('assignDuties.dutyPanel.personnel')}
          </Typography>
        </Box>
        ...
        <Box sx={{ px: 2, py: 1, borderTop: '1px solid', borderColor: 'divider' }}>
          {showAddLeaveType ? (
            <Stack spacing={1}>
              <TextField
                size="small"
                placeholder={t('assignDuties.leavePanel.leaveTypeName')}
              ...
              <Stack direction="row" spacing={0.5}>
                ...
                >
                  {addingLeaveType ? t('assignDuties.leavePanel.adding') : t('app.add')}
                </Button>
                <Button
                  size="small"
            ...
            >
              {t('assignDuties.leavePanel.addLeaveType')}
            </Button>
          )}
        </Box>
      </Paper>
      ...
      <Box sx={{ flex: 1, minWidth: 0, alignSelf: 'stretch' }}>
        {selectedCategoryData ? (
            ...
            <Box sx={{ px: 3, py: 2, bgcolor: sectionBgColor, borderBottom: '1px solid', borderColor: 'divider' }}>
              <Stack direction="row" spacing={1.5} alignItems="center" justifyContent="space-between">
                <Stack direction="row" spacing={1.5} alignItems="center">
                  ...
                  <Chip
                    label={`${selectedRecords.length} ${t('assignDuties.dutyPanel.personnel')}`}
                    size="small"
                    sx={{ fontWeight: 600, bgcolor: `${sectionColor}15`, color: sectionColor }}
                  />
                </Stack>
                <Button
                  ...
                  sx={{ textTransform: 'none' }}
                >
                  {t('assignDuties.leavePanel.addPersonOnLeave')}
                </Button>
              </Stack>
              <Typography variant="caption" color="text.secondary">
                {t(selectedCategoryData.descKey)}
              </Typography>
            </Box>
            ...
            <Box sx={{ flex: 1, overflowY: 'auto', p: 2 }}>
              {loading ? (
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                    <thead>
                      <tr style={{ borderBottom: '2px solid #e0e0e0' }}>
                        <th style={{ textAlign: 'left', padding: '8px 12px', fontWeight: 700, fontSize: '0.75rem', textTransform: 'uppercase', color: '#666' }}>{t('assignDuties.leavePanel.tableHeaders.badgeId')}</th>
                        <th style={{ textAlign: 'left', padding: '8px 12px', fontWeight: 700, fontSize: '0.75rem', textTransform: 'uppercase', color: '#666' }}>{t('assignDuties.leavePanel.tableHeaders.name')}</th>
                        <th style={{ textAlign: 'left', padding: '8px 12px', fontWeight: 700, fontSize: '0.75rem', textTransform: 'uppercase', color: '#666' }}>{t('assignDuties.leavePanel.tableHeaders.startDate')}</th>
                        <th style={{ textAlign: 'left', padding: '8px 12px', fontWeight: 700, fontSize: '0.75rem', textTransform: 'uppercase', color: '#666' }}>{t('assignDuties.leavePanel.tableHeaders.endDate')}</th>
                        <th style={{ textAlign: 'center', padding: '8px 12px', fontWeight: 700, fontSize: '0.75rem', textTransform: 'uppercase', color: '#666' }}>{t('assignDuties.leavePanel.tableHeaders.days')}</th>
                        <th style={{ textAlign: 'left', padding: '8px 12px', fontWeight: 700, fontSize: '0.75rem', textTransform: 'uppercase', color: '#666' }}>{t('assignDuties.leavePanel.tableHeaders.reason')}</th>
                        <th style={{ textAlign: 'center', padding: '8px 12px', fontWeight: 700, fontSize: '0.75rem', textTransform: 'uppercase', color: '#666' }}>{t('assignDuties.leavePanel.tableHeaders.actions')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {selectedRecords.map((record) => {
                        const isHighlighted = highlightedPersonId === record.personnelId;
                        return (
                        <tr 
                          key={record.id} 
                          style={{ 
                            ...
                            boxShadow: isHighlighted ? '0 0 12px rgba(234, 179, 8, 0.4)' : undefined,
                          }}
                          className={isHighlighted ? 'highlight-pulse' : undefined}
                        >
                          ...
                          <td style={{ padding: '8px 12px' }}>{record.endDate || t('assignDuties.leavePanel.ongoing')}</td>
                          ...
                          <td style={{ padding: '8px 12px', textAlign: 'center' }}>
                            <Box sx={{ display: 'inline-flex', gap: 0.5 }}>
                              {/* Close Leave button - only for leaves without end date */}
                              {!record.endDate && (
                                <Tooltip title={t('assignDuties.leavePanel.closeLeave')}>
                                  <IconButton
                                    size="small"
                                    onClick={() => { setCloseLeaveRecord(record); setCloseLeaveEndDate(''); setCloseLeaveComments(''); }}
                                    disabled={saving}
                                    sx={{ color: '#1d4ed8' }}
                                  >
                              ...
                              <Tooltip title={t('assignDuties.leavePanel.cancelLeave')}>
                                <IconButton
                                  size="small"
                ...
                <Typography variant="body2" color="text.secondary" sx={{ py: 2 }}>
                  {t('assignDuties.leavePanel.noActiveRecords')}
                </Typography>
              )}
            </Box>
          </Paper>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', opacity: 0.5 }}>
            <Typography variant="body2" color="text.secondary">
              {t('assignDuties.leavePanel.selectLeaveType')}
            </Typography>
          </Box>
        )}
      </Box>
      ...
      <Dialog open={addDialogOpen} onClose={() => setAddDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{t('assignDuties.leavePanel.addPersonOnLeave')} — {t(LEAVE_CATEGORIES.find(c => c.name === selectedCategory)?.nameKey || '')}</DialogTitle>
        <DialogContent>
          <Stack spacing={2.5} sx={{ mt: 1 }}>
            <Box>
              ...
              {formData.personnelId ? (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, p: 1.5, bgcolor: '#f0fdf4', borderRadius: 2, border: '1px solid #bbf7d0' }}>
                  <Box sx={{ flex: 1 }}>
                    <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, color: '#166534' }}>
                      {formData.personnelName || `Person ID: ${formData.personnelId}`}
                    </Typography>
                  </Box>
                  <Button size="small" onClick={() => setFormData((prev) => ({ ...prev, personnelId: null, personnelName: '' }))}
                    sx={{ textTransform: 'none', fontSize: '0.75rem', color: '#dc2626' }}>
                    Change
                  </Button>
                </Box>
              ) : (
                <PersonnelAutocomplete
                  ...
                  onSelectWithDetails={(personnelId, personName, badgeId) => setFormData((prev) => ({ ...prev, personnelId, personnelName: `${personName} (${badgeId})` }))}
                  onCancel={() => setFormData((prev) => ({ ...prev, personnelId: null, personnelName: '' }))}
                />
              )}
            </Box>
            <TextField
            ...
            <TextField
              ...
              helperText={t('assignDuties.leavePanel.leaveBlankForOpenEnded')}
            />
            <TextField
        ...
        <DialogActions>
          <Button onClick={() => { setAddDialogOpen(false); setFormData({ personnelId: null, personnelName: '', startDate: '', endDate: '', reason: '' }); }}>
            {t('app.cancel')}
          </Button>
          <Button
            variant="contained"
            onClick={handleSubmitLeave}
            disabled={saving || !formData.personnelId || !formData.startDate}
          >
            {saving ? <CircularProgress size={18} /> : t('app.submit')}
          </Button>
        </DialogActions>
      </Dialog>

      {/* Close Leave Dialog */}
      <Dialog open={closeLeaveRecord !== null} onClose={() => setCloseLeaveRecord(null)} maxWidth="xs" fullWidth>
        <DialogTitle sx={{ color: '#1d4ed8' }}>{t('assignDuties.closeLeaveDialog.title')}</DialogTitle>
        <DialogContent>
          {closeLeaveRecord && (
            <Stack spacing={2} sx={{ mt: 1 }}>
              <Box sx={{ bgcolor: '#f9fafb', borderRadius: 1, p: 1.5, border: '1px solid #e5e7eb' }}>
                <Typography variant="caption" color="text.secondary">{t('assignDuties.closeLeaveDialog.personnel')}</Typography>
                <Typography variant="body2" fontWeight={600}>{closeLeaveRecord.personnelName}</Typography>
                <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' }}>{t('assignDuties.closeLeaveDialog.leaveType')}</Typography>
                <Typography variant="body2">{closeLeaveRecord.leaveType.replace(/_/g, ' ')}</Typography>
                <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' }}>{t('assignDuties.closeLeaveDialog.startDate')}</Typography>
                <Typography variant="body2">{closeLeaveRecord.startDate}</Typography>
              </Box>
              <TextField
                ...
                required
                InputLabelProps={{ shrink: true }}
                inputProps={{ min: closeLeaveRecord.startDate }}
        ...
        <DialogActions>
          <Button onClick={() => setCloseLeaveRecord(null)}>{t('app.cancel')}</Button>
          <Button
            variant="contained"
            onClick={handleCloseLeaveConfirm}
            disabled={saving || !closeLeaveEndDate}
            sx={{ bgcolor: '#1d4ed8', '&:hover': { bgcolor: '#1e40af' } }}
          >
            {saving ? <CircularProgress size={18} /> : t('assignDuties.closeLeaveDialog.closeLeave')}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
